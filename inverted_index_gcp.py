...
class MultiFileReader:
    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            blob = bucket.get_blob(f"{self._folder_name}/{f_name}")
#             blob = bucket.get_blob(f'postings_gcp/{f_name}')
            ...
            n_read = builtins.min(offset + n_bytes, BLOCK_SIZE - offset)
            b.append(pl_to_read)
            n_bytes -= n_read
        return b''.join(b)
...
class InvertedIndex:  
    def __init__(self, docs={}):
        ...
        self.docs_norm = defaultdict(list)
    ...
    def add_doc(self, doc_id, tokens):
        ...
        self.docs_norm = norm([item[1] for item in w2cnt.items()])
    ...
    def posting_lists_iter(self, bucket_name,folder_name):
